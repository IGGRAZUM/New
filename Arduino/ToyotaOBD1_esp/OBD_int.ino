    ...
    while (bits > 0)  {
      if (BitCount < 4)  {
        if (BitCount == 0)
          ID = 0;
        ID >>= 1;
        if (state == MY_HIGH)  // inverse state as we are detecting the change!
          ID |= 0x08;
        ...
        if (bitpos == 0)      {
          ...
          if ((BitCount > 4) && (state != MY_LOW))  { // inverse state as we are detecting the change!
            ToyotaFailBit = BitCount;
            InPacket = false;
            break;
          } // end if ((BitCount > 4) && (state !=MY_LOW))
